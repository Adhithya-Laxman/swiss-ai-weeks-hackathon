...
class MarioAdventure {
    ...
    init() {
        ...
        // Check if we're returning from chatbot (has saved state)
        const stateLoaded = this.loadGameState();
    ...
    setupEventListeners() {
        ...
        document.getElementById('backToHomeButtonAlways').addEventListener('click', () => {
            window.location.href = '/';
        });
    }
    ...
    closeChat() {
        ...
        // Redirect to chatbot with disaster context
        if (this.currentLocation && this.disasterTitles[this.currentLocation.name]) {
            const disasterTitle = this.disasterTitles[this.currentLocation.name];
            const disasterContext = this.getDisasterContext(this.currentLocation.name);
            
            // Save game state before going to chatbot
            this.saveGameState();
            
            // Redirect to chatbot with disaster context
            window.location.href = `/game-chatbot?disaster=${encodeURIComponent(disasterTitle)}&context=${encodeURIComponent(disasterContext)}`;
        }
    }
    
    getDisasterContext(locationName) {
        // Map location names to disaster context from the JSON data
        const contextMap = {
            'Bolivia': 'Bolivia Wildfires: Relief Phase Initiated - National emergency declared, requesting international support for equipment, humanitarian aid, and technical assistance.',
            'Afghanistan': 'Earthquake in Nangarhar Province, Afghanistan - 6+ magnitude earthquake with severe humanitarian impact, over 2,000 injured and 6,000 homes destroyed.',
            'Yemen': 'Southern Yemen Flooding - Response Overview - Severe flooding affecting 100,000 people, destroying homes and critical infrastructure.',
            'Congo': 'Ebola Outbreak in Democratic Republic of Congo - Health authorities declared outbreak in Kasai Province with 28 suspected cases and 15 deaths.'
        };
        
        return contextMap[locationName] || 'Disaster management assistance needed.';
    }
    ...
    saveGameState() {
        ...
        const gameState = {
            ...
            mushroomCount: this.mushroomCount,
            completedLocations: this.completedLocations,
            marioPosition: {
                x: this.mario.x,
                y: this.mario.y,
                targetX: this.mario.targetX,
                targetY: this.mario.targetY,
                moving: this.mario.moving
            },
            locations: this.locations
        };
        
        sessionStorage.setItem('marioGameState', JSON.stringify(gameState));
    }
    
    loadGameState() {
        // Load game state from sessionStorage
        const savedState = sessionStorage.getItem('marioGameState');
        if (savedState) {
            const gameState = JSON.parse(savedState);
            this.gameStarted = gameState.gameStarted;
            this.mushroomCount = gameState.mushroomCount;
            this.completedLocations = gameState.completedLocations;
            
            // Restore Mario position
            this.mario.x = gameState.marioPosition.x;
            this.mario.y = gameState.marioPosition.y;
            this.mario.targetX = gameState.marioPosition.targetX;
            this.mario.targetY = gameState.marioPosition.targetY;
            this.mario.moving = gameState.marioPosition.moving;
            ...
            // Clear the saved state after loading (so next fresh visit starts clean)
            sessionStorage.removeItem('marioGameState');
    ...
    resetToInitialState() {
        // Always reset to initial state when game loads
        this.gameStarted = false;
        this.mushroomCount = 0;
        this.completedLocations = 0;
        ...
        // Reset all locations to not completed
        Object.values(this.locations).forEach(location => {
            location.completed = false;
        });
